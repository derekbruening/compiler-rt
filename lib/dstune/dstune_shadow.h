...
//===----------------------------------------------------------------------===//
//
// This file is a part of ThreadSanitizer (Dstune), a race detector.
//
// Shadow memory mappings for the dstune run-time.
//===----------------------------------------------------------------------===//
...
//
// Our shadow memory is a 1B-to-1B mapping (1Byte-to-1Byte).
...
// The resulting shadow memory regions are:
//
...
uptr appToShadow(uptr App) {
  DCHECK(IsAppMem(x));
  return (App & 0x00000fffffffffffull) + 0x0000120000000000ull;
}

ALWAYS_INLINE
bool isShadowMem(uptr Mem) {
  // We assume this is only really used for debugging and so there's
  // no need to hardcoded the mapping results.
#ifdef __x86_64__
